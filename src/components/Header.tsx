...
export function Header({ onOpenAbout }: HeaderProps) {
  const t = useT();
  return (
    <header className="flex flex-col items-center gap-1.5 px-6 pt-6 text-center sm:pt-8 lg:gap-2.5 lg:pt-10 [@media(max-height:720px)]:gap-1! [@media(max-height:720px)]:pt-3!">
      <div className="flex items-center gap-3">
        <Tunduk size={30} className="text-accent lg:hidden" />
        <Tunduk size={40} className="text-accent hidden lg:block" />
        <h1 className="font-serif text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl [@media(max-height:720px)]:text-3xl!">
          {t.app.title}
        </h1>
      </div>
      <p className="text-ink-muted text-xs sm:text-sm lg:text-base [@media(max-height:680px)]:hidden">
        {t.app.tagline}
      </p>
